...
{
    LOG_TEST(
        getLogger("MetadataStorageFromPlainObjectStorageCopyOperation"),
    ...
    if (!path_map.existsLocalPath(directory_from))
        throw Exception(
            ErrorCodes::FILE_DOESNT_EXIST, "Metadata object for the source directory path '{}' does not exist", path_from);

    const auto directory_to = path_to.parent_path();
    if (!path_map.existsLocalPath(directory_to))
        throw Exception(
            ErrorCodes::FILE_DOESNT_EXIST, "Metadata object for the destination directory path '{}' does not exist", path_to);

    object_storage->copyObject(
            StoredObject(remote_path_from),
            StoredObject(remote_path_to),
            getReadSettings(),
            getWriteSettings());
...
void MetadataStorageFromPlainObjectStorageCopyOperation::undo(std::unique_lock<SharedMutex> & /*metadata_lock*/)
{
    ...
    LOG_TRACE(getLogger("MetadataStorageFromPlainObjectStorageCopyOperation"), "Reversing file copying from '{}' to '{}'", path_from, path_to);
